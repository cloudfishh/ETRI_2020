...
result_filelist = [f for f in os.listdir('result_201027') if f.endswith('_result.csv')]
temp = pd.DataFrame([])
for f in result_filelist:
    # temp = pd.concat([temp, pd.read_csv(f'result_201027/{f}', index_col=0)], axis=0)
    temp = pd.concat([temp, pd.read_csv(f'result_201027/{f}')], axis=0)
...
if label[cum_Err] == 3:
    ...
    for ii in range(0, int(np.max(cum_len))):
        LI_wc[ii] = Rv[int(cum_Err)+St-1]+(Rv[int(cum_Err)+St+np.max(cum_len)]-Rv[int(cum_Err)+St-1])*(ii+1)/int(
            np.max(cum_len)+1)
else:
    LI_wc = np.zeros([int(np.max(cum_len)), ])
    for ii in range(0, int(np.max(cum_len))):
        LI_wc[ii] = Rv[int(cum_Err)+St]+(Rv[int(cum_Err)+St+np.max(cum_len)]-Rv[int(cum_Err)+St])*(ii)/int(
            np.max(cum_len))
...
plt.plot(range(int(cum_Err), int(cum_Err+np.max(cum_len))), PwoC[(Real_len == 1) & (label >= 2)], '-mv', linewidth=1,
         markersize=12)
plt.plot(range(int(cum_Err), int(cum_Err+np.max(cum_len))), LI_wc, '-cP', linewidth=1, markersize=12)
plt.plot(range(int(cum_Err), int(cum_Err+np.max(cum_len))), PwC[(Real_len == 1) & (label >= 2)], '-rd', linewidth=1,
         markersize=12)
...
plt.plot([int(cum_Err), int(cum_Err)],
         [PwoC[(Real_len == 1) & (label >= 2)][0], PwoC[(Real_len == 1) & (label >= 2)][0]], 'ks', linewidth=.7,
         markersize=12)
...
A42 = (label[:len(label)-4] == 4) & (label[1:len(label)-3] == 2) & (label[2:len(label)-2] == 2) & (
            label[3:len(label)-1] == 2)
A32 = (label[:len(label)-4] == 3) & (label[1:len(label)-3] == 2) & (label[2:len(label)-2] == 2) & (
            label[3:len(label)-1] == 2)
