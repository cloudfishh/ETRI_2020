...
if __name__=='__main__':
    ...
    accuracy = pd.DataFrame([], columns=dataset_1h.columns, index=['RMSE', 'MSE', 'MAE', 'MAPE'])
    result_df = pd.DataFrame([], columns=dataset_1h.columns)
    for c in range(len(dataset_1h.columns)-5):
        s_col = dataset_1h.iloc[:, c]
        ...
        result['pred'] = y_pred
        result.to_csv(f'200923_{dataset_1h.columns[c]}.csv')
        result_df[dataset_1h.columns[c]] = result['pred']

        print(f'{dataset_1h.columns[c]} COMPLETED')
        print(f'RMSE: {mean_squared_error(y_true, y_pred)**(1/2)}')
        print(f' MSE: {mean_squared_error(y_true, y_pred)}')
        print(f' MAE: {mean_absolute_error(y_true, y_pred)}')
        print(f'MAPE: {mape(y_true, y_pred)}\n')
        accuracy[dataset_1h.columns[c]] = [mean_squared_error(y_true, y_pred)**(1/2),
        ...
        plt.title(f'{dataset_1h.columns[c]}')
        plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.show()
        plt.savefig(f'200923_{dataset_1h.columns[c]}.png')

    accuracy.to_csv('200923_lstm_accuracy.csv')
    result_df.to_csv('200923_lstm_result.csv')
