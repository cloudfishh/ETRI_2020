...
# test_idx_list = [1147, 3222, 4771, 7326, 14363]       # 24
test_idx_list = [6400, 1093, 8435, 7014, 6402, 3151, 1142]         # 1
for test_idx in test_idx_list:
    ...
    fcst_bidirec24, _ = linear_prediction(trn_x, trn_y, tst_x, f_len_fwd=24, f_len_bwd=24)
    fcst_forward1, _ = linear_prediction(trn_x, trn_y, tst_x, f_len_fwd=1, f_len_bwd=0)
    fcst_forward48, _ = linear_prediction(trn_x, trn_y, tst_x, f_len_fwd=48, f_len_bwd=0)
    fcst_backward1, _ = linear_prediction(trn_x, trn_y, tst_x, f_len_fwd=0, f_len_bwd=1)
    fcst_backward48, _ = linear_prediction(trn_x, trn_y, tst_x, f_len_fwd=0, f_len_bwd=48)

    idx_inj = np.where(test_mask == 2)[0]
    plt.rcParams.update({'font.size': 10})

    plt.figure(figsize=(7.5, 5))
    plt.plot(data_col.values[idx_inj[0]-48:idx_inj[-1]+24], '.-', color='black')
    plt.plot(range(48, 48 + nan_len), fcst_forward1, '^-', linewidth=1, alpha=1, color='royalblue')
    plt.plot(range(48, 48 + nan_len), fcst_forward48, 'x-', linewidth=1, alpha=1, color='cornflowerblue')
    plt.plot(range(48, 48 + nan_len), fcst_backward1, 'P-', linewidth=1, alpha=1, color='seagreen')
    plt.plot(range(48, 48 + nan_len), fcst_backward48, '*-', linewidth=1, alpha=1, color='mediumseagreen')
    plt.plot(range(48, 48 + nan_len), fcst_bidirec1, 'X-', linewidth=1, color='tomato')
    plt.plot(range(48, 48 + nan_len), fcst_bidirec24, 'o-', linewidth=1, color='orangered')
    plt.legend(['observation',
                'forward, 1', 'forward, 48',
                'backward, 1', 'backward, 48',
                'bidirectional (1,1)', 'bidirectional (24,24)'],
               loc='upper right')
    plt.xticks(ticks=[i for i in range(0, 48 + 24 + nan_len, 12)],
